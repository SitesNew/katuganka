...
$back="<center>Поверніться <a href='javascript:history.back(1)'><B>назад</B></a>"; // Удобная строка
...
echo'<BR><div style="width:50%"><div style="float:left;">Захисний код :</div>';
...
<META name='Description' content='$boardm_info краща безкоштовна дошка об'яв.'>
...
<body>
<h1>Дошка об'яв міні - Board mini</h1><center><DIV>
<a href='$mainpage'><button class='knopka knopka-red'>Головна Сайту</button></a>
<a href='$boardm_url'><button class='knopka knopka-red'>Головна Дошки Об'яв</button></a>
<a href='index.php?add'><button class='knopka knopka-red'>Додати Об'яву</button></a>
<div><FORM action='index.php?find' method=post>Пошук: <INPUT name=findme> <input type=submit class=mainoption value=' ПОШУК '></form></div>
...
if ($findme=="" || mb_strlen($findme)<$minfindme) error("Ваш запит пустий, або меньше чим $minfindme символів");
...
do {$i--;
    ...
    if (mb_strlen($stroka)>"2") {
        ...
        $stroka=str_replace("<br>"," &nbsp;", $stroka);
        if (!isset($m)) {print "<BR>По запросу '<U><B>$findme</B></U>' найдено:<BR><BR><table border=1 width=100%><TR height=25 align=center class=small bgColor=#cccccc><TD><B>№</B></TD><TD width=100><B>Имя</B></TD><TD width=300><B>Заголовок</B></TD><TD width=*><B>короткий зміст об'яви</B></TD></TR>"; $m="1"; }
        $number++;
        $msgnumber=$i-1;
...
if (!isset($m)) print "По вашому запиту нічого не знайдено.";
if (!isset($m)) print "<BR><BR><BR>Специфіка пошуку:<BR> - пошукач простий, тому використовуйте слово ибо частину слова. Не рекомендуем використовувати фрази.<BR> - пошук ведеться тільки по тексту об'яви.<BR><BR>
Обмеження на пошук : <BR>- мінімальна кільк.  символів: <B>$minfindme</B>.<BR>";
echo "</table><BR>";
}
}
...
<div class='set_item2'>
    <div class='boxmax'>
		...
		<p><small>Об'яву розмістив:</small> $name</p>
		<P><small>Дата подачі: $dt[6]<BR> Дата видалення: $deldate</small><BR></p>
		<font size=+1>Діє: <font size=+2 color=navy>$tektday/$firstday</font> дн.</font>
		</P>
    </div>
</div>";

$navmsg=null; // БЛОК НАВИГАЦИИ ВПЕРЁД-НАЗАД для быстрого перемещения по объявлениям с 02.2023
if ($last!=-1) {$dtlast=explode("|",$lines[$last]); $navmsg.="<div class='wr_block'><a class='wr_knopki dark' title='Попередня об'ява' href='index.php?id=$dtlast[10]'>&#9668; попередня об'ява</a></div>";}
    $navmsg.="<div class='wr_block'><a class='wr_knopki red' title='Повернутись на Головну Сторінку' href='$boardm_url'>Повернутися</a></div>";
if ($next!=-1) {$dtnext=explode("|",$lines[$next]); $navmsg.="<div class='wr_block'><a class='wr_knopki blue' title='Наступна об'ява' href='index.php?id=$dtnext[10]'>наступна об'ява &#9658;</a></div>";}
...
<div id='dalee-main'><P>Дякую, <B>$name</B>, Ваша об'ява  успішно додана.<BR>Через кілька секунд Вы будете автоматически переміщені <BR><B>на першу сторінку дошки об'яв.</B></P>
<div id='dalee-start'><div id='dalee-ramka'><div id='dalee-zalivka'><div id='dalee-line'></div></div></div></div><BR>
<B><a href='$loc'>ДАЛІ >>></a></B></div></body></html>"; exit;}
...
<center><h2>Додати об'яву</h2>
<B>Им'я</B>: <input type=text placeholder='Ваше им'я' value='' name=name size=26 style='width:38%'>&nbsp;
E-mail: <input type=text placeholder='email@site.ru' value='' name=email size=26 style='width:38%'><br><br>
<B>Заголовок об'яви</B>: <input type=text placeholder='Тема об'яви' value='' name=zag size=56 style='width:64%'><br><br>";

if ($use_www==TRUE) print"Сайт: <input placeholder='http://ваш-сайт.ru' type=text value='' name=www style='width:84%'><br><br>";

print"<B>Текст об'яви:</B> (від 100 до $maxmsg символів)<br><textarea cols=55 rows=16 size=2000 name=msg style='width:90%;height:120px;'></textarea><br><br>
<input type=button class=button value=' B ' style='font-weight:bold; width: 30px' onclick=\"DoSmilie(' [b]  [/b] ');\">&nbsp; <input type=button class=button value=' RB ' style='font-weight:bold; color:red' onclick=\"DoSmilie('[RB] [/RB]');\">&nbsp; <input type=button class=button value=' IMG ' style='font-weight:bold; color:navy' onclick=\"DoSmilie('[img][/img]');\"> - спец. виділення підвищує рейтинг об'яви в поукових системах.<br>

<B>Термін зберігання об'яв</B>: (7-365 днів) <SELECT name=days><OPTION value=7>7 днів</OPTION><OPTION value=14>14 днів</OPTION><OPTION selected value=30>30 днів</OPTION><OPTION value=60>60 днів</OPTION><OPTION value=90>90 днів</OPTION><OPTION value=365>365 днів</OPTION></SELECT><BR><BR>";

if ($antispam==TRUE) nospam(); // АНТИСПАМ

print"</center><center><input type='submit' value=' Додати '></center><br></div>";
}
...
$pageinfo.="<div style='padding:6px;' class=pgbutt>Сторінки: &nbsp;";
...
<div class='set_item'>
    <div class='box2'>
		<h2><a href='index.php?id=$dt[10]' title='$dtitle'>$dt[3]</a></h2>
       	<p style='min-height: 220px;'>$dt[5]</p>
		<p><small>Об'яву № <B>$num</B> розмістив:</small> $name</p>
		<P><small>Дата подачі: $dt[6]<BR> Дата видалення: $deldate</small><BR></p>
		<font size=+1>Діє: <font size=+2 color=navy>$tektday / $firstday</font> дн.</font>
		</P>
    </div>
...
print"</div><div style='clear:both;'>$pageinfo <BR>Всього об'яв : <B>$itogo</B><BR>";
