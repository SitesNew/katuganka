...
// Определяем URL скрипта, подставялем https
$host=$_SERVER["HTTP_HOST"]; $self=$_SERVER["PHP_SELF"]; $boardm_url=str_replace('index.php','',"https://$host$self");
...
do {$i--;
    ...
    if (mb_strlen($stroka)>"2") {
        ...
        $stroka=preg_replace ("#([^\[img\]])(http|https|ftp|goper):\/\/([a-zA-Z0-9\.\?&=\;\-\/_]+)([\W\s<\[]+)#i", "\\1<noindex><a href=\"\\2://\\3\" target=\"_blank\">\\2://\\3</a></noindex>\\4", $stroka);
...
if ($use_www==TRUE) print"Сайт: <input placeholder='https://ваш-сайт.ru' type=text value='' name=www style='width:84%'><br><br>";
...
if (mb_strlen($dt[5])>400) {$dt[5]=mb_substr($dt[5],0,380); $dt[5]=preg_replace("#([^\[img\]])(http|https|ftp|goper):\/\/([a-zA-Z0-9\.\?&=\;\-\/_]+)([\W\s<\[]+)#i", "\\1<noindex><a href=\"\\2://\\3\" target=\"_blank\">ссылка</a></noindex>\\4", $dt[5]); $dt[5].="... <a href='index.php?id=$dt[10]'> Подробнее...</a>";}
else $dt[5]=preg_replace("#([^\[img\]])(http|https|ftp|goper):\/\/([a-zA-Z0-9\.\?&=\;\-\/_]+)([\W\s<\[]+)#i", "\\1<noindex><a href=\"\\2://\\3\" target=\"_blank\">ссылка</a></noindex>\\4", $dt[5]);
...
?>
<br><center><small>Powered by <a href='https://www.wr-script.ru/'>WR-Board mini</a> &copy; 2.3 UTF-8</small></body></html>